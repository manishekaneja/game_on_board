...
class Game extends React.Component {
  ...
  listenerFunction(_) {
      ...
      switch (_.code) {
        case "KeyD":
          ...
          return this.props.enqueueSnackbar(
            "You Decided to Defend Yourself...Smart Work",
            {
              variant: "info",
              anchorOrigin: {
                vertical: "bottom",
                horizontal: this.state.first_player_turn ? "left" : "right",
              },
            }
          );
        case "KeyA":
          this.attackAction();
          return this.props.enqueueSnackbar(
            "You Decided To Attack...Aggresive",
            {
              variant: "info",
              anchorOrigin: {
                vertical: "bottom",
        ...
        default:
          return this.props.enqueueSnackbar("Confirm the Action First", {
            variant: "error",
            anchorOrigin: {
              vertical: "bottom",
              horizontal: this.state.first_player_turn ? "left" : "right",
    ...
    let playercell = this.state.boardDetails.filter((cell) =>
      this.state.first_player_turn ? cell.hasPlayer1 : cell.hasPlayer2
    );
  ...
  componentDidMount() {
    this.props.enqueueSnackbar("Let's Start", {
      variant: "success",
    ...
    this.setState({
      boardDetails: board_values(
        ...
        this.initalizePowerUps()
      ),
    });
    this.listenerEvent = window.addEventListener(
      "keydown",
  ...
  getUniquePoints(count) {
    ...
    for (let x = 0; x < count; ++x) {
      ...
      if (
        points.filter((e) => e.row === c.row && e.column === c.column)
          .length === 0
      ) {
        points.push(c);
      } else {
  ...
  updateWhen(new_row, new_col, old_row, old_col, render) {
    let next_cell = this.state.boardDetails.filter(
      (cell) => cell.row === new_row && cell.column === new_col
    );
    if (
      new_row < 0 ||
      new_row > 7 ||
      ...
      (next_cell.length > 0 &&
        (next_cell[0].hasRock ||
          next_cell[0].hasPlayer1 ||
          next_cell[0].hasPlayer2))
    ) {
      return this.props.enqueueSnackbar("Can't Perform that Move", {
        variant: "warning",
        anchorOrigin: {
          vertical: "bottom",
          horizontal: this.state.first_player_turn ? "left" : "right",
    ...
    if (this.state.turn_count <= 0) this.checkNeighboursCells(new_row, new_col);
    this.setState((ps) => {
      let turn_count,
        first_player_turn,
        presentLocationPower = next_cell[0].powerUp;
      this.props.enqueueSnackbar("You made a Move", {
        variant: "default",
        anchorOrigin: {
          vertical: "bottom",
          horizontal: ps.first_player_turn ? "left" : "right",
        },
      });
      if (ps.turn_count > 0) {
        ...
        turn_count = 2;
        first_player_turn = !ps.first_player_turn;
        this.props.enqueueSnackbar("Now it's your Turn", {
          variant: "info",
          anchorOrigin: {
            vertical: "bottom",
            horizontal: first_player_turn ? "left" : "right",
          },
        });
      }
      return {
        turn_count,
        first_player_turn,
        boardDetails: ps.boardDetails.map((cell) =>
          cell.row === old_row && cell.column === old_col
            ? {
                ...cell,
                hasPlayer1: false,
                hasPlayer2: false,
            ...
            : cell.row === new_row && cell.column === new_col
            ? {
                ...cell,
        ...
        player_1: {
          ...
          ...(ps.first_player_turn
            ? {
                weapon_power: presentLocationPower
                  ? presentLocationPower
                  : ps.player_1.weapon_power,
              }
            : {}),
        },
  ...
  checkNeighboursCells(row, col) {
    let neighbouringPlayer = this.state.boardDetails.filter(
      (cell) =>
  ...
  getUpdatedProfile(attacker, victim, victim_key) {
    return {
      ...
      [victim_key]: {
        ...victim,
        strength:
          victim.strength - attacker.weapon_power / (victim.defence ? 2 : 1),
        defence: false,
      },
    };
  }
  ...
  defenceAction() {
    this.setState((ps) =>
      !ps.first_player_turn
        ? { player_1: this.turnOnDefence(ps.player_1), openDialog: false }
        : { player_2: this.turnOnDefence(ps.player_2), openDialog: false }
    );
  }
  attackAction() {
    ...
    return (
      <>
        ...
        {!this.state.game_ended ? (
          ...
          >
            <Board
              first_player_turn={this.state.first_player_turn}
              details={this.state.boardDetails}
            />
          </div>
        ) : (
          <div
          ...
          >
            <Card style={{ maxWidth: 400 }}>
              ...
              >
                ...
                >
                  <span style={{ marginRight: 2 }}>Retry</span>
                  <ThreeSixtyIcon />
                </Button>
              </CardActions>
            </Card>
    ...
    return (
      <Paper
        style={{
          ...
          backgroundImage: `url(${values.green})`,
        }}
      >
        {this.props.details.map((block, index) => (
          <BoardCell
            first_player_turn={this.props.first_player_turn}
            key={index}
            isBlock={block.hasRock}
            powerUp={block.powerUp}
            hasPlayer1={block.hasPlayer1}
            hasPlayer2={block.hasPlayer2}
            render={block.render}
          />
        ))}
      </Paper>
    ...
    return (
      <span
        style={{
          ...
          color: "black",
          backgroundColor:
            (this.props.hasPlayer1 && this.props.first_player_turn) ||
            (this.props.hasPlayer2 && !this.props.first_player_turn)
              ? this.props.powerUp > 0
                ? "lightseagreen"
                : "yellow"
              : this.props.powerUp > 0
              ? "lightgreen"
      ...
      >
        {this.props.render ||
          (this.props.powerUp > 0 && weapon_render(this.props.powerUp))}
      </span>
    );
  }
}
...
class DefenceOption extends React.Component {
  render() {
    return (
      ...
      >
        <DialogTitle>What Do You Wish For???</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Either You Can Attack Your Opponent With your Full Power
          </DialogContentText>
          <DialogContentText style={{ textAlign: "center" }}>
            OR
          </DialogContentText>
          <DialogContentText>
            Instead of Attacking You can choose to defend Your Self but Reduce
            the damage recived to Half.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
